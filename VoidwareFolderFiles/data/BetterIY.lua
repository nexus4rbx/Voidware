...
-- Credits to all who develop Infinite Yield. Didn't develop this, just added support for the new chat system :p
...
TextChatService.MessageReceived:Connect(function(messagetab) -- hopefully new text chat system support gets addded to the official IY :omegalol:
	...
	if plr == Players.LocalPlayer then 
		for i,v in pairs(cmds) do 
			if messagetab.Text and messagetab.Text:sub(1, v.NAME:len() + 1):lower() == prefix..""..v.NAME:lower() then 
				do_exec(messagetab.Text, Players.LocalPlayer)
				break
			end
		end
	end
